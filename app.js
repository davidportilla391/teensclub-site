...
const TC_EVENTS = {
  vol01: {
    pretitle: 'Vol.01 · 26 Jun · La Calle de las Sirenas',
    title: 'Tu acceso.',
...
async function goToCheckout() {
  ...
  if (!cover) return;
  if (!nameIsValid() || !rpIsValid()) { refreshPayState(); return; }

  // Guardar orden localmente con folio único.
  // El registro a Google Sheets se hace desde gracias.html al finalizar el pago.
  const order = {
  ...
  try {
    // Llamar a la Netlify Function para crear la preferencia en Mercado Pago
...
const sections = ['inicio', 'eventos', 'covers', 'info', 'ubicacion', 'faq'];
...
function onScroll() {
  ...
  if (window.scrollY > window.innerHeight * 0.7) stickyBuy.classList.add('show');
  else stickyBuy.classList.remove('show');
}
